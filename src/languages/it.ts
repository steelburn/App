...
(e.g. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
        ...
        plusMore: ({count}: {count: number}) => `[it] +${count} more`,
    ...
    readOnlyConversation: '[it] This conversation is read-only.',
    reportAction: {
        asCopilot: '[it] as copilot for',
        assistedBy: (agentName: string) => `[it] assisted by ${agentName}`,
        humanSupportAgent: '[it] a human support agent',
    ...
    iou: {
        ...
        addExistingExpense: '[it] Add existing expense',
        selectExistingExpense: '[it] Select at least one expense to add to the report.',
        emptyStateExistingExpenseTitle: '[it] No existing expenses',
        emptyStateExistingExpenseSubtitle: '[it] Looks like you don’t have any existing expenses. Try creating one below.',
        addExistingExpenseConfirm: '[it] Add to report',
        ...
        bulkDuplicateLimit: `[it] You can duplicate up to ${CONST.SEARCH.BULK_DUPLICATE_LIMIT} expenses at a time. Please select fewer expenses and try again.`,
        taxDisabledAlert: {
    ...
    initialSettingsPage: {
        ...
        troubleshoot: {
            ...
            showBranchNameInTitle: '[it] Show branch name in browser title',
        ...
        help: '[it] Help',
        helpPage: {
            title: '[it] Got questions?',
            description: "[it] We're here to help, around the clock.",
    ...
    cardPage: {
        ...
        spendRules: '[it] Spend rules',
        editSpendRules: '[it] Edit spend rules',
        reviewTransaction: '[it] Review transaction',
...
${amount} for ${merchant} - ${date}`,
    ...
    workflowsExpensesFromPage: {
        ...
        memberAlreadyInWorkflowTitle: '[it] Member already in a workflow',
        memberAlreadyInWorkflowPrompt: ({memberName, approverName}: {memberName: string; approverName: string}) =>
            `[it] ${memberName} is already in an approval workflow that submits to ${approverName}. Adding them here will move them to this workflow.`,
    },
                ...
                values: {
                    ...
                    [CONST.NETSUITE_EXPORT_DESTINATION.JOURNAL_ENTRY]: {
                        ...
                        nonReimbursableDescription: dedent(`
                            ...
                            If you'd like to set a specific vendor for each card, go to *Settings > Domains > Company Cards*.
                        `),
                        travelDescription: '[it] Travel expenses will export as journal entries to the NetSuite account specified below.',
                    },
                },
...
3. Once the feed is enabled, click Submit below and we’ll work on adding it.`,
        ...
        hr: {
            title: '[it] HR',
            subtitle: '[it] Connect HR tools and keep employee approvals in sync.',
            settingsTitle: '[it] Gusto settings',
            syncStageName: ({stage}: SyncStageNameConnectionsParams) => {
                switch (stage) {
                    case 'startingImportGusto':
                        return '[it] Importing Gusto data';
                    case 'gustoSyncLoadCompany':
                        return '[it] Loading Gusto company data';
                    case 'gustoSyncImportEmployees':
                        return '[it] Importing employees';
                    case 'gustoSyncBuildApprovalChains':
                        return '[it] Building approval chains';
                    case 'gustoSyncFinalize':
                        return '[it] Finalizing sync';
                    case 'jobDone':
                        return '[it] Waiting for imported data to load';
                    default: {
                        return `[it] Translation missing for stage: ${stage}`;
            ...
            gusto: {
                title: '[it] Gusto',
                connect: '[it] Connect',
                connectionDescription: '[it] Connect Gusto to keep employee approvals in sync with your workspace.',
                approvalMode: '[it] Approval mode',
                finalApprover: '[it] Final approver',
            },
        },
        export: {
...
Require expense details like receipts and descriptions, set limits and defaults, and automate approvals and payments – all in one place.`,
            ...
            hr: {
                title: '[it] HR integrations',
                description:
                    '[it] Connect your HR provider to automatically sync employees and manage approval workflows. Keep your team roster and reporting structure up to date without manual work.',
                onlyAvailableOnPlan: ({formattedPrice, hasTeam2025Pricing}: {formattedPrice: string; hasTeam2025Pricing: boolean}) =>
                    `[it] <muted-text>HR integrations are only available on the Control plan, starting at <strong>${formattedPrice}</strong> ${hasTeam2025Pricing ? `[it] per member per month.` : `[it] per active member per month.`}</muted-text>`,
            },
            travel: {
                title: '[it] Travel',
        ...
        rules: {
            ...
            customRules: {
                ...
                policyDocument: '[it] Policy document',
                policyText: '[it] Policy text',
            },
            spendRules: {
                title: '[it] Spend',
...
Add more spend rules to protect company cash flow.`,
                ...
                summaryMoreCount: ({summary, count}: {summary: string; count: number}) => (count > 0 ? `[it] ${summary}, +${count} more` : summary),
                summaryMerchants: ({
                    merchants,
                    hiddenCount,
                    shownCount,
                    ...
                    `${action === CONST.SPEND_RULES.ACTION.BLOCK ? '[it] Blocked' : '[it] Allowed'} ${shownCount > 1 ? '[it] merchants' : '[it] merchant'}: ${merchants}${hiddenCount > 0 ? `[it] , +${hiddenCount} more` : ''}`,
                summaryCategories: ({
                    categories,
                    hiddenCount,
                    shownCount,
                    ...
                    `${action === CONST.SPEND_RULES.ACTION.BLOCK ? '[it] Blocked' : '[it] Allowed'} ${shownCount > 1 ? '[it] categories' : '[it] category'}: ${categories}${hiddenCount > 0 ? `[it] , +${hiddenCount} more` : ''}`,
    ...
    workspaceActions: {
        ...
        deleteCustomUnitRate: (customUnitName: string, rateName: string) => `[it] removed the "${customUnitName}" rate "${rateName}"`,
        ...
        updatedManualApprovalThreshold: (oldLimit: string, newLimit: string) => `[it] changed the manual approval limit for all expenses to ${newLimit} (previously ${oldLimit})`,
        ...
        updateCustomTaxName: (oldName: string, newName: string) => `[it] changed the custom tax name to "${newName}" (previously "${oldName}")`,
        updateCurrencyDefaultTax: (oldName: string, newName: string) => `[it] changed the workspace currency default tax rate to "${newName}" (previously "${oldName}")`,
        updateForeignCurrencyDefaultTax: (oldName: string, newName: string) => `[it] changed the foreign currency default tax rate to "${newName}" (previously "${oldName}")`,
    ...
    search: {
        tabs: {
            ...
            expenses: '[it] Expenses',
            submit: '[it] Drafts',
            approve: '[it] Needs approval',
            pay: '[it] Ready to pay',
            accounting: '[it] Accounting',
            export: '[it] Awaiting export',
            unapprovedCash: '[it] Cash accruals',
            unapprovedCard: '[it] Card accruals',
            statements: '[it] Card statements',
            reconciliation: '[it] Bank reconciliation',
        ...
        bulkActions: {
            ...
            duplicateReport: ({count}: {count: number}) => `[it] Duplicate ${count === 1 ? '[it] report' : '[it] reports'}`,
    ...
    report: {
        ...
        actions: {
            type: {
                ...
                integrationSyncFailedRecurrence: ({count}: {count: number}) => `[it] (Repeated ${count} times.)`,
    ...
    subscription: {
        ...
        details: {
            ...
            creditBalance: '[it] Credit balance',
        ...
        cancelSubscription: {
            title: '[it] Cancel subscription',
            subtitle: '[it] What’s the main reason you’re canceling your subscription?',
...
Here’s a *test receipt* to show you how it works:`,
    ...
    domain: {
        ...
        common: {
            ...
            error: "[it] Couldn't save this change. Please try again.",
        },
        groups: {
            title: '[it] Groups',
            memberCount: () => {
            ...
            defaultGroup: '[it] Default group for new members',
            defaultGroupPrompt: (currentName: string, newName: string) =>
                `[it] Are you sure you want to make ${newName} the default group? New members will be invited to this group instead of the previous default group (${currentName}). `,
            makeDefault: '[it] Make default',
            neverMind: '[it] Never mind',
        },
    },
};
