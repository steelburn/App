...
这里有一张*测试收据*，来展示它是如何运作的：`,
    ...
    domain: {
        ...
        groups: {
            ...
            RestrictExpenseWorkspaceCreation: '限制费用工作区的创建/删除',
            RestrictExpenseWorkspaceCreationDescription: '防止成员创建费用工作区或将自己从费用工作区中移除。',
            RestrictDefaultLoginSelection: '限制默认登录选择',
            RestrictDefaultLoginSelectionDescription: '防止成员将登录邮箱更改为公司域名以外的地址。',
            StrictlyEnforceWorkspaceRules: '严格执行工作区规则',
            StrictlyEnforceWorkspaceRulesDescription: '提交报告前必须满足所有工作区规则。不允许手动例外。',
            ...
            defaultGroupPrompt: (currentName: string, newName: string) => `您确定要将${newName}设为默认群组吗？新成员将被邀请到此群组，而不是之前的默认群组（${currentName}）。 `,
            makeDefault: '设为默认',
            nevermind: '算了',
        },
    },
